...
def test_when_initialization_equivalence_then_output_matches_linear_core():
    ...
    out_features = 4
    rank = 2
    model = BatchMomentumLiquidLN(in_features, out_features, rank=rank, bias=True)
    device = model.linear_core.weight.device

    # Check that buffers start at zero placeholder shapes
...
def test_when_forward_pass_then_buffers_accumulate_momentum_per_batch_element():
    torch.manual_seed(42)
    in_features = 8
    out_features = 4
    rank = 2
    ...
    # Momentum buffer shape matches the batch dimensions: (B, T, rank, feat)
    assert a_raw_1.shape == (2, 3, rank, out_features)
...
def test_when_optimizing_then_updates_core_hypernet_and_buffers():
    ...
    # Buffer b_raw should now update after forward pass
    _ = model(x)
    assert not torch.all(model.b_raw == 0)


def test_when_batch_size_changes_then_buffers_resized_dynamically():
    torch.manual_seed(42)
    in_features = 8
    out_features = 4
    rank = 2
...
def test_when_core_frozen_then_backward_only_updates_hypernetwork():
    ...
    in_features = 8
    out_features = 4
    model = BatchMomentumLiquidLN(in_features, out_features, rank=2, bias=True)
    device = model.linear_core.weight.device
...
def test_when_input_shape_mismatched_then_raises_runtime_error():
    in_features = 8
    out_features = 4
    model = BatchMomentumLiquidLN(in_features, out_features, rank=2)
    device = model.linear_core.weight.device
