...
def test_when_initialization_equivalence_then_output_matches_linear_core():
    ...
    in_features = 8
    out_features = 4
    model = StableLiquidLN(in_features, out_features, rank=3, bias=True, dynamic_bias=True)
...
def test_when_conditioning_provided_then_uses_cond_features():
    ...
    in_features = 8
    out_features = 4
    model = StableLiquidLN(in_features, out_features, rank=3, bias=True)
    device = model.linear_core.weight.device
    ...
    # Test that different conditioning inputs produce different outputs
...
def test_when_input_shape_mismatched_then_raises_runtime_error():
    in_features = 8
    out_features = 4
    model = StableLiquidLN(in_features, out_features, rank=3)
    device = model.linear_core.weight.device
