...
so this task gives a _training-budget failure_, not a clean architectural
...
- **The attention-free ablation behaves correctly.** Every `_noattn` variant
  fails `overwrite_recall` (success_rate = 0). That is _expected_: the task
...
RAM, no CUDA): the `tiny` preset (n*embd=128, `ours` 2 layers / `baseline` 4
...
`benchmarks/llm_bench_report*\*.json`; aggregate in
...
3. **`CrossAttnLoraLN` has the best LAMBADA ppl (135,305) and 2nd-best train
   ...
   expensive _intermediary_ (~6% above the other `ours` configs) yet still
...
6. **Param budget is dominated by the embedding.** At `tiny`, the 50,257×128
   ...
   (`baseline` is actually ~90K _larger_ than `ours` because it uses 4 layers).
